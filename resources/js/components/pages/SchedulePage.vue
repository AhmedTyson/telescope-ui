<template>
    <div>
        <h1 class="text-2xl font-bold text-[#f4f4f5] dark:text-white mb-6">Schedule</h1>
        <FilterPanel :active-count="Object.keys(getActiveFilters()).length" @search="search" @reset="reset">
            <div>
                <label class="block text-xs font-medium text-[#a1a1aa] dark:text-[#71717a] mb-1">Schedule Command</label>
                <input v-model="filters.schedule_command" type="text" placeholder="Search by command..." class="input-field text-xs">
            </div>
            <div>
