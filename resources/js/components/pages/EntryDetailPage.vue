<template>
    <div>
        ...
        <div v-if="loading" class="flex items-center justify-center py-24">
            <div class="flex items-center gap-3 text-[#a1a1aa] dark:text-[#71717a]">
        ...
        <template v-else-if="entry">
            ...
            <!-- User -->
            <div v-if="userInfo" class="bg-transparent  rounded-lg border border-telescope-border  overflow-hidden mb-6">
                <div class="px-6 py-4">
                    <h2 class="text-lg font-semibold text-[#f4f4f5] dark:text-white mb-3">User</h2>
                    <div class="space-y-2">
                        <div v-for="(val, key) in userLabeled" :key="key" class="flex items-baseline gap-3">
                            <span class="text-xs font-medium text-[#a1a1aa] dark:text-[#71717a] w-40 flex-shrink-0">{{ key }}</span>
                            <span class="text-sm text-[#f4f4f5]  font-mono">{{ val }}</span>
                        </div>
                    </div>
                </div>
            </div>
